...
function OutputHistogram({ data, outputTitle }) {
  ...
  useEffect(() => {
    ...
    const outputData = Object.values(data).map(
    ...
    outputTitles.forEach((outputTitle) => {
      const outputValues = outputData.map(
        (experimentOutputs) => experimentOutputs[outputTitle],
      );
      ...
      y.domain([0, d3.max(histogram, (d) => d.length) + 1]);
